...
impl HealthServer {
    ...
    pub async fn set_metric(&self, key: &str, value: &str) {
        self.metrics.write().await.insert(key.to_string(), value.to_string());
    }
                ...
                Ok((mut stream, addr)) => {
                    ...
                    tokio::spawn(async move {
                        ...
                        let (status, body) = if first_line.starts_with("GET /health") {
                            ...
                            let metrics_json: Vec<String> = m.iter()
                                .map(|(k, v)| format!("\"{}\":\"{}\"", k, v))
                                .collect();

                            let status_code = if is_ready { "200 OK" } else { "503 Service Unavailable" };
